...
(
	...
	// 输入数据与权重矩阵重合部分，5*5
	wire signed [7:0] m04,m14,m24,m34,m44;//这些信号是 wire 类型，是因为它们直接从输入端口 taps 引入，并且没有存储或时序逻辑上的操作
...
//====================conv 模块工作时钟计数===================	
	...
	begin
		if(sum_valid)
			if(cnt2 == Ni-1) // 即 0~27， 24,25,26,27多的4步用于纵向移动一步时的窗口同步
				cnt2<=10'd0;  //eg. 在28x28的图像中，当 cnt2 达到27时，表示该行的卷积操作完成，接着开始下一行的卷积操作。
			else
				cnt2 <= cnt2 + 10'd1;
		else
			cnt2<=10'd0;
	...
	always@(posedge clk)
	if(sum_valid)
	// cnt2s 是一个行步长控制计数器，它控制窗口在纵向上的步长移动
