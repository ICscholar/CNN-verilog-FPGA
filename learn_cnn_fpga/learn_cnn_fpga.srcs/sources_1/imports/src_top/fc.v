...
(
	...
	//6个输入的选择主要是基于计算并行性、硬件资源优化、项目需求等多个因素综合考虑的结果
	...
	reg sum_valid;
	reg wren;
	reg signed [15:0] p0,p1,p2,p3,p4,p5;//两个八位的数据相乘最大需要16位
	reg signed [16:0] sum00,sum01,sum02;//两个16位数据相加最大需要17位
	reg signed [17:0] sum10,sum11;//两个1位数据相加最大需要18位
	reg signed [18:0] sum;
	...
	begin
		if(!rstn)
			dout_r <= 0;
		else if(wren)        //当 wren 信号有效时，表示一次完整的加权求和计算已经完成，此时将 dout_r 重置为 0，为下一次累加做好准备
			dout_r <= 0;
		else if(ivalid_ff_3)
	...
	// 打拍采沿
	always@(posedge clk)//四级流水线，索引将输入有效信号延迟4拍
	begin
		ivalid_ff_0 <= ivalid;
	...
	begin
	case(cnt)
	//size of output from maxpooling(2*2) is 12*4*4=192 
	//channel 1: 0-15
	//channel 2: 16-31
	//channel 3: 32-47
	//...
	//channel 12: 176-191
	// 并行度设计为2， 则12*4*4-> 2*6*4*4  目前将state 0 and 1两种状态对应为并行度
	4'd0:begin
		if(!state)
			begin
				p0 <= w[0] *din_0;    
